...
class FieldSpec:
    ...
    # KeyValueListWidget (ui/widgets/kv_list.py): row_group группирует поля
    # строки списка в несколько рядов (по умолчанию все поля — один общий
    # ряд, как раньше); narrow — ряд сжимает поле по ширине текста-подсказки
    # вместо растяжения (стрейч достаётся остальным полям ряда). Оба флага
    # аддитивны — существующие вызовы без row_group/narrow не меняются.
    narrow: bool = False
    # copy_btn: индивидуальная кнопка [КОП] рядом с полем строки списка (в
    # дополнение к общей [КОП] строки — полезно, когда поле не в первом ряду
    # row_group и общая кнопка до него не достаёт, УИ §2026-07-15). gen_btn
    # (УИ §2026-07-16, как в карточке аккаунта): поле получает пару кнопок ПОД
    # полями строки — «СГЕНЕРИРОВАТЬ ПАРОЛЬ»/«ПАРАМЕТРЫ ГЕНЕРАЦИИ» — из
    # KeyValueListWidget.gen_callback/gen_settings_callback (сам виджет
    # генератор не знает, см. ui/server_tabs.py). Оба флага действуют только
    # внутри KeyValueListWidget и аддитивны — существующие вызовы без них не
    # меняются.
    copy_btn: bool = False
    gen_btn: bool = False
